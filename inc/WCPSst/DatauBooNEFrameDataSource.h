...
namespace WCPSst {
     ...
     */
    class DatauBooNEFrameDataSource : public WCP::FrameDataSource {
      //mutable TTree* tree;	// or TChain
      //WCPSst::RootEvent event;

      public:
      DatauBooNEFrameDataSource(const char* root_file, const WCP::GeomDataSource& gds,int bins_per_frame1 = 9600, int inactiveNF = 0, int flag_add_noise = 0);
	DatauBooNEFrameDataSource(const TH2F *hu_raw, const TH2F *hv_raw, const TH2F *hw_raw, TTree *T_bad, TTree *T_lf, TTree *Trun, const WCP::GeomDataSource& gds);
